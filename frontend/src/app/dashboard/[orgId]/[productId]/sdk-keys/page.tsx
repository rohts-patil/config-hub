...
export default function SDKKeysPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [c, e] = await Promise.all([api.configs.list(productId), api.environments.list(productId)]);
        setConfigs(c); setEnvs(e);
  ...
  useEffect(() => {
    ...
    (async () => {
      try { setKeys(await api.sdkKeys.list(productId, selectedConfig)); }
      catch { setKeys([]); }
    })();
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">SDK Keys</h1><p className="text-muted-foreground">Keys for connecting SDKs to your configs</p></div>
        {configs.length > 0 && (
          <Select value={selectedConfig} onValueChange={(v) => v && setSelectedConfig(v)}>
            <SelectTrigger className="w-[200px]"><SelectValue placeholder="Select config" /></SelectTrigger>
      ...
      {keys.length === 0 ? (
        <Card><CardContent className="flex flex-col items-center justify-center py-12"><Key className="h-12 w-12 text-muted-foreground/50 mb-4" /><p className="text-muted-foreground">No SDK keys found for this config.</p></CardContent></Card>
      ) : (
        <Card>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {keys.map((k) => (
                <TableRow key={k.id}>
                  <TableCell className="font-mono text-sm">{k.key.slice(0, 12)}...{k.key.slice(-4)}</TableCell>
                  <TableCell>{envName(k.environment_id)}</TableCell>
                  <TableCell><Badge variant={k.revoked ? "destructive" : "default"}>{k.revoked ? "Revoked" : "Active"}</Badge></TableCell>
