...
export default function FlagListPage() {
  const { orgId, productId, configId } = useParams() as { orgId: string; productId: string; configId: string };
  ...
  const fetchData = async () => {
    try {
      ...
      setSettings(s);
      setEnvs(e);
      if (e.length > 0 && !selectedEnv) setSelectedEnv(e[0].id);
    } catch (err: any) { toast.error(err.message); }
    finally { setLoading(false); }
  };

  useEffect(() => { fetchData(); }, [configId, productId]);
    ...
    const fetchValues = async () => {
      ...
      await Promise.all(
        settings.map(async (s) => {
          try { vals[s.id] = await api.settings.getValue(configId, s.id, selectedEnv); }
  ...
  const handleCreate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await api.settings.create(configId, { name: newName, key: newKey, setting_type: newType, hint: newHint || undefined });
      setNewName(""); setNewKey(""); setNewType("boolean"); setNewHint(""); setDialogOpen(false);
  ...
  const handleToggle = async (setting: Setting) => {
    ...
    try {
      await api.settings.updateValue(configId, setting.id, selectedEnv, {
        ...
        targeting_rules: sv.targeting_rules?.map((r) => ({
          served_value: r.served_value, order: r.order,
          conditions: r.conditions.map((c) => ({
            condition_type: c.condition_type, attribute: c.attribute,
            comparator: c.comparator, comparison_value: c.comparison_value,
            segment_id: c.segment_id, prerequisite_setting_id: c.prerequisite_setting_id,
          })),
        })) || [],
        percentage_options: sv.percentage_options?.map((p) => ({
          percentage: p.percentage, value: p.value, order: p.order,
        })) || [],
      });
      setEnvValues((prev) => ({
        ...prev,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage feature flags for this config</p>
        </div>
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent>
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              <div className="space-y-2"><Label>Name</Label><Input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="My Feature" required /></div>
              <div className="space-y-2"><Label>Key</Label><Input value={newKey} onChange={(e) => setNewKey(e.target.value)} placeholder="my_feature" required /></div>
              ...
              <div className="space-y-2"><Label>Hint</Label><Input value={newHint} onChange={(e) => setNewHint(e.target.value)} placeholder="Optional hint" /></div>
      ...
      {filtered.length === 0 ? (
        <Card><CardContent className="flex flex-col items-center justify-center py-12">
          ...
          <p className="text-muted-foreground">{settings.length === 0 ? "No feature flags yet." : "No flags match your search."}</p>
        </CardContent></Card>
      ) : (
        <Card>
          <Table>
              ...
              {filtered.map((s) => {
                ...
                return (
                  <TableRow key={s.id}>
                    ...
                    <TableCell><code className="text-xs bg-muted px-1.5 py-0.5 rounded">{s.key}</code></TableCell>
                    <TableCell><Badge variant="secondary" className={typeColors[s.setting_type] || ""}>{s.setting_type}</Badge></TableCell>
                    <TableCell className="text-sm">{val !== undefined ? String(val) : "—"}</TableCell>
                    {selectedEnv && (
                      <TableCell>
                        {isBool && sv ? (
                          <Switch checked={!!val} onCheckedChange={() => handleToggle(s)} />
                        ) : "—"}
                      </TableCell>
                    )}
                    <TableCell>
                      <div className="flex gap-1">
                        <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => router.push(`/dashboard/${orgId}/${productId}/${configId}/${s.id}`)}>
