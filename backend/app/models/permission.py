...
class Tag(Base):
    ...
    settings: Mapped[List["SettingTag"]] = relationship(back_populates="tag", cascade="all, delete-orphan")
...
class AuditLog(Base):
    ...
    entity_type: Mapped[str] = mapped_column(String(50), nullable=False)  # setting, environment, etc.
...
class Webhook(Base):
    ...
    config_id: Mapped[Optional[str]] = mapped_column(String(36), ForeignKey("configs.id"), nullable=True)
